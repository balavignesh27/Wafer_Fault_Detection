...
class Prediction_Data_validation:
    ...
    def createDirectoryForGoodBadFinalRawData(self):
        ...
        try:
            ...
            if not os.path.isdir(path):
                os.makedirs(path)
            path = os.path.join('Prediction_Raw_files_validated/', 'Final_Validated_data/')
            if not os.path.isdir(path):
                os.makedirs(path)
    ...
    def deleteExistingFinalValidatedInputFile(self):
        ...
        """ Method Name: deleteExistingFinalValidatedPrectionFile
            Description: This method deletes the directory made to store the Final Validated Prediction data.
            Output: None
            On Failure: OSError  """
        ...
        try:
            ...
            if os.path.isdir(path+'Final_Validated_data/'):
                ...
                file = open('Prediction_logs/GeneralLog.txt', 'a+')
                self.logger.log(file, 'Final Prediction file directory deleted before starting validation')
                file.close()
    ...
    def finalValidatedInputFile(self):
        ...
        """ Method Name: finalValidatedInputFile
            Description: This method creates a directory of final Prediction File for prediction.
            Output: None
            On Failure: OSError  """

        try:
            file = open('Prediction_Logs/finalValidatedInputFile.txt', 'a+')
            ...
            dataset.to_csv('Prediction_Raw_files_validated/Final_Validated_data/PredictionFile.csv', index=None, header=True)
            file = open('Prediction_Logs/finalValidatedInputFile.txt', 'a+')
            self.logger.log(file, 'Final Validated Input File creation Successful!!')
            file.close()

        except OSError as s:
            file = open('Prediction/GeneralLog.txt', 'a+')
            self.logger.log(file, 'Error while deleting directory:%s' % s)
            file.close()
            raise OSError
